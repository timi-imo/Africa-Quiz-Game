...
document.getElementById('advanced').addEventListener('click', () => {
 ...
 document.getElementById('start-beginner').addEventListener('click', () => {
   ...
   document.getElementById("answer-beginner").addEventListener('keyup', answer => {
      var input = document.getElementById("answer-beginner").value.toLowerCase();
     
     if(countryNames.length == 0){
        //do something if person gets all?!
        return;
      }
      else if(countryNames.includes(input) || getAlternativeNamesBeginner(input)){
        ...
        if(input==='sudan'){
          ...
          counter = counter + 1;
          document.getElementById("score-beginner").innerHTML = `SCORE: ${counter}`;
          document.getElementById("game-over-score").innerHTML = `YOUR SCORE: ${counter}!`;
        }
        else{
...
const getRandomCountry = () => {
  ...
  document.querySelectorAll(`.${countryID.toLowerCase()}`).forEach(country => {
      country.style.opacity = "1";
      country.style.fill = "black";
    });
...
const getAlternativeNames = (currentName) => {
   if(currentName === 'swaziland'){
          ...
          var alternativeCountryNames = ['drc', 'democratic republic of the congo', 'République démocratique du Congo']
        }
          ...
          var alternativeCountryNames = ['saint helena', 'ascension', 'tristan da cunha']
        }
        else if(currentName === 'réunion'){
          var alternativeCountryNames = ['reunion']
        }
        else if(currentName=== 'côte divoire'){
          var alternativeCountryNames = ['cote divoire', 'ivory coast', 'cote d\'ivoire', 'côte d\'ivoire'];
        }
  ...
  if(name === 'eswatini' && countryNames.includes('swaziland')){
    ...
    if(countryNames.includes('congo (democratic republic of the)')){
      ...
      return false;
        ...
        else if(name ==='saint helena' || name==='ascension' || name==='tristan da cunha'){
      ...
      return false;
        ...
        else if(["cote divoire", "ivory coast", "cote d'ivoire", "côte d'ivoire"].includes(name)){
        ...
        else if(name === 'guinea bissau' && countryNames.includes('guinea-bissau')){
...
const keyupEvent = () => { 
    ...
    if(input === currentName || alternativeNames.includes(input)){
        ...
        document.getElementById("score-beginner").innerHTML = `SCORE: ${counter2}`;
        document.getElementById("game-over-score").innerHTML = `YOUR SCORE: ${counter2}!`;
        document.querySelectorAll(`.${currentCode.toLowerCase()}`).forEach(country => {
